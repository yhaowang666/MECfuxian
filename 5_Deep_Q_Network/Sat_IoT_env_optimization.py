# -*- codeing = utf-8 -*-
# @Time : 2021/1/11 14:53
# @Author : 王浩
# @File : Sat_IoT_env_optimization.py
# @Software : PyCharm
import numpy as np
# import time
# import sys
...
class Sat_IoT(object):
    ...
    def _build_Sat_IoT(self):
        ...
        self.A2 = np.arange(Gat_N+1)      # 0代表由卫星计算，或者由网关计
    ...
    def Source_allocation(self):
        ...
        for i in range(K):
            # 需要分配的总的卫星通信资源
            if self.a[0][i] != 0:
                sat_num = int(self.a[0][i]) - 1
                self.X_allocation[0][sat_num] += np.sqrt(self.U[i][0] * eta * w_k)
                # 判断需要分配的总的卫星计算资源
                if self.a[1][i] == 0:
                    self.Z_allocation[0][int(self.a[0][i]) - 1] += np.sqrt(self.U[i][0] * eta * w_k)
                # 判断需要分配的总的网关通信和计算资源
                else:
                    gat_num = int(self.a[1][i]) - 1
                    self.Gat_allocation[0][gat_num] += np.sqrt(self.U[i][0] * eta * w_k)
    ...
    def Source_update(self):
        ...
        for i in range(K):
            # 更新X,Y,Z,Q,U
            sat_num = int(self.a[0][i])
            gat_num = int(self.a[1][i])
            if sat_num != 0:
                # 如果任务在该时隙处理，则将总任务集当中对应任务的数据大小放在由卫星有关的任务集
                self.omega[i][sat_num - 1] = self.U[i][0]
                self.omega_t[i][sat_num - 1] = self.U[i][0]
                # 分配卫星上的通信资源
                self.X[i][sat_num - 1] = 2/3 * self.X_remain[0][sat_num - 1] \
                    * np.sqrt(self.U[i][0] * eta * w_k) / self.X_allocation[0][sat_num - 1]
                # 计算等待时延和卫星地面之间的传输时延
                self.time_reward += self.time*Time_slot + self.U[i][0]/self.X[i][sat_num - 1]
                # 计算由IoT终端和卫星之间的传播时延
                self.time_reward += 2 * self.PL[sat_num - 1][np.random.randint(0, Ter_N)] / c_v
                # 判断由卫星计算还是网关计算
                if gat_num == 0:
                    # 由卫星计算的任务集
                    self.psi[i][sat_num - 1] = self.U[i][0]
                    self.psi_c[i][sat_num - 1] = self.U[i][0]
                    # 计算由卫星处理的能量消耗
                    self.energy_reward += self.U[i][0] * cycles_average_bit * Energy_cost
                    # 分配卫星上的计算资源
                    self.Z[i][sat_num - 1] = 2/3 * self.Z_remain[0][sat_num - 1] \
                        * np.sqrt(self.U[i][0] * eta * w_k) / self.Z_allocation[0][sat_num - 1]
                    # 计算由卫星处理的计算时延
                    self.time_reward += self.U[i][0] / (self.Z[i][sat_num - 1] / cycles_average_bit)
                else:
                    # 与网关有关的任务集
                    self.phi[i][gat_num - 1] = self.U[i][0]
                    self.phi_t[i][gat_num - 1] = self.U[i][0]
                    self.phi_c[i][gat_num - 1] = self.U[i][0]
                    # 分配网关通信资源
                    self.Y[i][gat_num - 1] = 2/3 * self.Y_remain[0][gat_num - 1] \
                        * np.sqrt(self.U[i][0] * eta * w_k) / self.Gat_allocation[0][gat_num - 1]
                    # 计算卫星网关端的传输时延
                    self.time_reward += self.U[i][0] / self.Y[i][gat_num - 1]
                    # 分配网关计算资源
                    self.Q[i][gat_num - 1] = 2/3 * self.Q_remain[0][gat_num - 1] \
                        * np.sqrt(self.U[i][0] * eta * w_k) / self.Gat_allocation[0][gat_num - 1]
                    # 计算网关处理的计算时延和传播时延
                    self.time_reward += self.U[i][0] / self.Q[i][gat_num - 1] + \
                        2 * (self.PL[sat_num - 1][gat_num+Ter_N-1])/c_v
                # 并将U中该位置的任务清零
                self.U[i][0] = 0
        # if self.U.sum() != 0:
    ...
    def update(self):
            ...
            for j in range(Sat_N):
                ...
                if self.omega_t[i][j] <= 0:
                    if self.omega_t[i][j] == 0:
                        ...
                        for m in range(Gat_N):
                            if self.phi_t[i][m] != 0:
                                self.phi_t[i][m] -= self.Y[i][m] * (-self.omega_t[i][j] / self.X[i][j])
                    self.omega_t[i][j] = 0
                    self.omega[i][j] = 0
                    self.X[i][j] = 0
        ...
        for i in range(K):
            for j in range(Gat_N):
                if self.phi_t[i][j] <= 0:
                    if (self.phi_t[i][j] == 0) & (self.Q[i][j] > 0):
    ...
    def step(self, action):
        ...
        self.show_system()
        return self.state.reshape(1, self.n_features), self.reward, done
